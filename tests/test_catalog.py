...
def test_catalog_default_deny_transitions_and_listing(tmp_path, monkeypatch):
    ...
    with TestClient(module.app) as client:
        ...
        assert client.get("/api/kids/catalog/items/by-video/video-1").json()["state"] == "candidate"

        approved_source = client.patch(
            f"/api/kids/sources/{source_data['id']}/state",
            json={"state": "approved", "actor": "parent", "reason": "reviewed", "correlation_id": "t1"},
        )
        ...
        approved_item = client.patch(
            ...
            json={"state": "approved", "actor": "parent", "reason": "reviewed", "correlation_id": "t2"},
        )
        assert approved_item.json()["state"] == "approved"
        assert [x["video_id"] for x in client.get("/api/kids/catalog/items").json()["items"]] == ["video-1"]
        assert client.get("/api/kids/catalog/revision").json()["revision"] == 4

        revoked = client.patch(
            f"/api/kids/catalog/items/{item_data['id']}/state",
            json={"state": "revoked", "actor": "parent", "reason": "withdrawn", "correlation_id": "t3"},
        )
        assert revoked.json()["state"] == "revoked"
        assert client.get("/api/kids/catalog/items").json()["items"] == []
        reapprove = client.patch(
            f"/api/kids/catalog/items/{item_data['id']}/state",
            json={"state": "approved", "actor": "parent", "reason": "again", "correlation_id": "t4"},
        )
